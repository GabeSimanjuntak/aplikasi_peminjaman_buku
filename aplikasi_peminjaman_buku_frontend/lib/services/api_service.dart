...
class ApiService {
  ...
  static Future<Map<String, dynamic>> kembalikanBuku(int idPeminjaman) async {
    ...
    final res = await http.post(
      ...
      headers: {"Authorization": "Bearer $token", "Accept": "application/json"},
      body: {"id_peminjaman": idPeminjaman.toString()},
    );
  ...
  static Future<Map<String, dynamic>> pinjamBuku(int id) async {
    ...
    final response = await http.post(
      Uri.parse("$baseUrl/peminjaman"),
      headers: {"Authorization": "Bearer $token", "Accept": "application/json"},
      body: {"id_user": userId.toString(), "id_buku": id.toString()},
    );
  ...
  // ===============================
  static Future<Map<String, dynamic>> ajukanPengembalianWithDate(
    int id,
    DateTime tanggal,
  ) async {
    final prefs = await SharedPreferences.getInstance();
  ...
  static Future<dynamic> updatePeminjamanStatus(
    int idPeminjaman,
    String status,
  ) async {
    try {
  ...
  static Future<Map<String, dynamic>> cancelPeminjaman(int id) async {
    final url = "$baseUrl/peminjaman/$id/cancel";
    final headers = await getHeaders();
    ...
    if (response.statusCode == 200) {
      ...
      throw Exception("Gagal membatalkan peminjaman: ${response.body}");
    }
  }
}
